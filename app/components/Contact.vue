    ...
    <div class="flex bg-primary w-full px-4 py-12 justify-center">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-12 max-w-6xl mx-auto">
            <div class="col-span-1 text-primary-foreground flex flex-col justify-center">
                <h2 class="text-3xl font-bold mb-6 text-center">Start Your Digital Transformation</h2>
                <p class="text-lg text-center">Let's discuss how I can elevate your technology infrastructure
                </p>
            </div>
            <form class="grid-cols-1 bg-card rounded-xl shadow-xl p-6" @submit.prevent="sendEmail">
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <!-- Last Name -->
                    <div class="col-span-2 md:col-span-1">
                        <label for="last-name" class="block text-sm">Last
                            Name</label>
                    ...
                    <div class="col-span-2">
                        <label for="email" class="block text-sm">Email
                            Address</label>
                    ...
                    <div class="col-span-2">
                        <button type="submit"
                            class="w-full py-2 px-4 border border-transparent shadow-sm text-sm rounded-md bg-primary text-primary-foreground hover:bg-primary/90 cursor-pointer">
                            Send Message
                        </button>
                    </div>
                </div>
...
const sendEmail = async () => {
    try {
        ...
        if (response.status === 200) {
            status.value = 0;
            statusMessage.value = "Email sent!";
        } else {
